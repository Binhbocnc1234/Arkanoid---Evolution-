<img width="1200" height="300" alt="gameLogo" src="https://files.catbox.moe/ykxybz.png" />

# Arkanoid-Evolution-
A simple brick-breaking game, where your goal is to break all the bricks with just a ball and a paddle.
Game Project for OOP class INT2202_9.

## Game Demo
https://youtu.be/WezhIkPEwsg

## Screenshots
<img width="1094" height="816" alt="image" src="https://files.catbox.moe/gxqmnf.webp" />
<img width="1088" height="805" alt="image" src="https://files.catbox.moe/hyhywa.webp" />

## File structure & UML Diagram
src/main/java/game: Main game files: Objects, Managers,..
src/main/resource: Game resources.

<img width="2092" height="1512" alt="Arkanoid drawio" src="https://files.catbox.moe/y2cgw0.png" />
https://app.diagrams.net/#G1znOCjc9x6hAgiLjNYmYmy8sOicMaGXE-#%7B%22pageId%22%3A%22CjH1w59uPpAKWMkMjewL%22%7D

## Contributors
- <b>Nguyễn Thanh Bình: Project Leader</b>

Manage basic game functions and structure.
Manage game logics (Collision, moving objects,...)

- Tô Đức Anh
  
Powerups and powerup management system.
  
- Trần Tuấn Cường
  
Bricks and brick management system.
Level system.

- Trần Đức Diễn

Ball and paddle system.
Sound management system.

## How To Play
It's simple: Break all bricks and advance forward!
Try staying alive for as long as possible to get the max Score!

### Ball & Paddle
Guide your ball with your paddle toward the bricks.
Be careful, if you lose your ball, that's a Game Over! Your score will be resetted to 0 and you will have to do that level again.

### Bricks
There are many types of bricks for you to break:

- Normal: The basic one. Can only tank 1 hit.
- Steel: More strudy than normal brick, can tank another hit.
- Gold: Extra hard, can soak up 3 damage.
- Diamond: The hardest and rarest one. Can take up to 4 damage.
- Obsidian: Basically indestructable.
- TNT: Will help you by creating a 4-way blast upon being destroyed! All bricks affected will take 1 damage.

### Powerups
Beside that, there are also some Powerups to assist you through the levels. They will be reseted every level though.

- Amplify: Extend your paddle's size.
- Spilt: Summon 2 more balls.
- Piercing: Balls now can go through bricks and ignore collision.

## Level select
10 levels for you to enjoy! Initially you are at level 1, to unlock level 2 you need to break all the bricks, and so on. Each level will have a different brick arrangement
<img width="1095" height="811" alt="image" src="https://files.catbox.moe/qvgqmq.webp" />

## LocalMultiplayer
Feeling a little bored playing alone? No problem, 2 player mode is now available, each person will control a different paddle. The first player uses the A and D keys, the second player uses the arrow keys.

## Other features
- Catchy tunes and sound effects! You can adjust volume when pause menu is appearing

## Prerequisites
This game requires Java Runtime Environment to run. You can download it [here](https://www.java.com/en/download/manual.jsp).

## Download & Launch
Simply run the .exe file.

# Acknowledges & Credits
### This game was built with [Apache Maven](https://maven.apache.org/).

## Sound Credits
- Menu BGM: [No Event Theme (From Dynamix)](https://www.youtube.com/watch?v=99u18_Im3gU)
- Game BGM: [SCTL - Cervelle Connexion](https://www.youtube.com/watch?v=vuM5S_QtcEc)
- Sound effects were taken from various sources online.













